use crate::{Object, utility, utility::constants::*, utility::debug::*, utility::share, utility::structures::*, utility::window::{ProgramProc, VulkanApp}};
...
impl VulkanApp27 {
    pub fn new(event_loop: &winit::event_loop::EventLoop<()>, object: &Object) -> VulkanApp27 {
        let window =
        ...
        let (vertices, indices) = VulkanApp27::load_model(object);
    ...
    fn load_model(object: &Object) -> (Vec<VertexV3>, Vec<u32>) {
        let mut vertices = vec![];
        let mut indices = vec![];
        ...
        for face in &object.mesh.faces {
            for vertex in face {
                let vertex = VertexV3 {
                    ...
                    color: [1.0, 1.0, 1.0, 1.0],
                    tex_coord: [vertex.2[0], vertex.2[1]],
                };
                vertices.push(vertex);
            }
        }
        indices = (0..object.mesh.faces.len() as u32 * 3).collect();
...
pub fn main(object: &Object) {
    let program_proc = ProgramProc::new();
    let vulkan_app = VulkanApp27::new(&program_proc.event_loop, object);
