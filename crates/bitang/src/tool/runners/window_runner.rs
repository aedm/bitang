...
use crate::engine::{
    BitangImage, FrameContext, GpuContext, PixelFormat, RenderStage, Size2D, SwapchainImage, Viewport, SCREEN_RENDER_TARGET_ID
};
...
impl egui_wgpu::CallbackTrait for CustomRenderCallback {
    fn paint(
        ...
        let inner: &Arc<Mutex<Option<AppInner>>> = callback_resources.get().unwrap();
        todo!()
    }
...
impl App {
    fn new(
        ...
        *inner.lock().unwrap() = Some(app_inner);
        ...
        render_state.renderer.write().callback_resources.insert(Arc::clone(&inner));
